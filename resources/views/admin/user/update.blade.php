	...
	<form action="/admin/user/{{ $user->id }}" method="post">
		...
		<select name="classunit_id" id="classunit_id">
			@foreach($classunits as $classunit)
				<option value="{{ $classunit->id }}"
						@if($user->classunit_id == $classunit->id) selected @endif>{{ $classunit->name }}</option>
			@endforeach
		</select><br>
		...
		<select name="samorzadType" id="samorzadType">
			...
			<option value="wiceprzewodniczacy" @if($user->samorzadType == "wiceprzewodniczacy") selected @endif>
				Wiceprzewodniczący
			</option>
			<option value="przewodniczacy" @if($user->samorzadType == "przewodniczacy") selected @endif>Przewodniczący
			</option>
